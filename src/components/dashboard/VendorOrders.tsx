...
export const VendorOrders: React.FC<VendorOrdersProps> = ({
  ...
  return (
    <div className="space-y-12 animate-in fade-in duration-700">
      ...
      <div className="bg-white rounded-[48px] border border-slate-100 shadow-xl overflow-hidden overflow-x-auto min-h-[400px]">
            ...
            {orders.length > 0 ? (
              orders.map((order, i) => (
                <tr key={order._id || i} className="hover:bg-slate-50/50 transition-all duration-300 group">
                  ...
                  <td className="px-10 py-8 font-bold text-slate-600 text-xs border-r border-slate-50">
                    {order.items && order.items.length > 0 ? (
                      <div className="flex flex-col gap-1.5">
                        <span className="font-black text-slate-900 uppercase text-[10px]">{order.items[0].name}</span>
                        {order.items.length > 1 && (
                          <span className="text-[9px] font-black text-white bg-slate-900 px-2 py-0.5 rounded-full w-fit uppercase tracking-tighter">+{order.items.length - 1} Collection Members</span>
                        )}
                      </div>
                    ) : (
